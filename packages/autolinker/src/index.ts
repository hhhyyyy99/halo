...
import { concatenateStrings, extractMatchesAndText, generateLinkHref, recursiveMatch } from './utils';
...
function link(input: string, element?: (match: string, type: Constants.MATCH_TYPE) => string) {
  const matches = list(input).map(item => {
    if (Verify.isTransformResult(item)) {
      ...
      const href = generateLinkHref(value, type);
...
export { link, list, recursiveMatch, concatenateStrings, extractMatchesAndText, generateLinkHref };
